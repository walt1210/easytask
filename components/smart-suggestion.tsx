...
interface SmartSuggestionProps {
  tasks: Task[]
  onApply: (taskId: string) => void // Updated to accept taskId
}
...
export function SmartSuggestion({ tasks, onApply }: SmartSuggestionProps) {
  ...
  return (
    <div className="bg-gradient-to-r from-yellow-50 to-orange-50 dark:from-yellow-950/20 dark:to-orange-950/20 rounded-xl p-4 border border-yellow-200/50 dark:border-yellow-800/30 flex items-center justify-between gap-4 flex-wrap animate-in fade-in slide-in-from-top-2">
      <div className="flex items-center gap-3">
        ...
        <p className="text-sm text-foreground">
          ...
          <span className="text-muted-foreground">
            {upcomingPersonalTask.title.split("—")[0].trim()} is due at {dueTime} — consider focusing on this next.
          </span>
        </p>
      </div>
      <button
        onClick={() => onApply(upcomingPersonalTask.id)} // Pass ID back to page.tsx
